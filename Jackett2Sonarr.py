...
def main():
    ...
    user_indexers = []
    path_sonarr = input(
        'Sonnar.db path ? :') or '/data/Dockers/TOOLS/Sonarr/sonarr.db'
    path_jackett = input(
        'Jackett configuration path ? :') or '/data/Dockers/TOOLS/Jackett/Jackett'
    ...
    onlyfiles = [f for f in listdir(path_indexers) if (
        isfile(join(path_indexers, f)) and (f.endswith('.json')))]
    ...
    if choice_all in 'yY':
        for index_name in av_files:
            ...
            if not all_line:
                sql_insert = (index_name, 'Torznab', '''{"baseUrl": "'''+url_jackett+'''/api/v2.0/indexers/'''+index_name+'''/results/torznab/","apiPath": "/api","apiKey": "''' +
                              api_key+'''","categories": [5000],"animeCategories": [5070],"animeStandardFormatSearch": true}''', 'TorznabSettings', 1, 1, 1, 25, '[]', 0)
                insert_datas(path_sonarr, sql_insert)
            else:
                sql_update = ('''Update Indexers set Name = "'''+index_name+'''", Settings = '{"baseUrl": "'''+url_jackett+'''/api/v2.0/indexers/'''+index_name +
                              '''/results/torznab/","apiPath": "/api","apiKey": "'''+api_key+'''","categories": [5000],"animeCategories": [5070],"animeStandardFormatSearch": true}'  where Name = "'''+index_name+'''";''')
                update_datas(path_sonarr, sql_update)
        exit()

    user_indexers = [int(item) for item in input(
        "Enter the Indexers number separate with spaces : ").split()]
